...
def cli(
    ...
    # Run security scan
    logger.info('Start active security scan of URL {website}'.format(
    ...
    if total_vulnerable == 0:
        logger.info('Summary: No security relevant AEM Dispatcher URLs found in {total} rules.'.format(
            total=total_scans))
        click.exit(0)
    else:
        logger.error('Summary: Found {hit} of {total} security relevant AEM Dispatcher URLs.\n\nVulnerable results are: \n{vulnerable_results}'.format(
