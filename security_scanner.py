...
class SecurityScanner():
    ...
    def __init__(self, host = 'http://localhost:8080', page_path = '/content/geometrixx/en', request_timeout = 10, resource_path = 'aem-sec-paths.txt'):
        '''
        Constructor
    ...
    def retrieve_dispatcher_invalidate_cache_response(self):
        ...
        return self.retrieve_path_response(path='/dispatcher/invalidate.cache', headers=headers)
    ...
    def validate_all_paths(self):
        ...
        for path in self.paths:
            print('\rScanning {count} of {total} paths.'.format(count=count, total=len(self.paths)), end='', flush=True)
            count += 1
    ...
    def retrieve_path_response_async(self, path, lock):
        ...
        lock.acquire()
        self.counter += 1
        print('\rScanning {count} of {total} paths.'.format(count=self.counter, total=len(self.paths)), end='', flush=True)
        lock.release()
