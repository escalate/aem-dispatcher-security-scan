...
class TestSecurityScanStatus(unittest.TestCase):
    ...
    def test_url(self):
        ...
        '''
        status = SecurityScanStatus('http://localhost:80', '/content/some-page/en', None)
    ...
    def test_is_vulnerable_none(self):
        ...
        self.assertFalse(status.is_vulnerable, 'Vulnerability is not found if response is None!')
    ...
    def test_is_vulnerable_200(self):
        ...
        self.assertTrue(status.is_vulnerable, 'Vulnerability is found if path exists (response status code is not 404)!')
    ...
    def test_is_vulnerable_404(self):
        ...
        self.assertFalse(status.is_vulnerable, 'Vulnerability is not found if path does not exist (response status code is 404)!')
    ...
    def test_str(self):
        ...
        status = SecurityScanStatus('http://localhost:80', '/content/some-page/en', response)

        self.assertRegex(str(status), '\[\d{4}\-\d{2}\-\d{2} \d{2}:\d{2}:\d{2}\.\d{3,}\] \d{3} \- http://localhost:80/content/some-page/en')
